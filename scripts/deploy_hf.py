"""Deploy this app to Hugging Face Spaces (Streamlit SDK) and print the live URL.
...
app_port: 7860
pinned: false
short_description: Chat with your docs — hybrid BM25 + embeddings RAG
---

# Hybrid Search RAG over Internal Docs

Upload PDFs / Markdown / text and chat with them. Retrieval fuses **BM25**
(keyword) with **NVIDIA Nemotron embeddings** (semantic) via Reciprocal Rank
Fusion; answers come from a free OpenRouter model with inline citations.
Uploads are processed in memory and scoped to your browser session.

Source: https://github.com/saiteja007-mv/hybrid-search-rag
"""
...
def main() -> int:
    ...
    ap.add_argument("--space-name", default="hybrid-search-rag")
    args = ap.parse_args()
    ...
    # Space README (with front-matter) replaces the project README on the Space.
    ...
    api.upload_folder(
        ...
        commit_message="Deploy hybrid-search-rag",
    )
